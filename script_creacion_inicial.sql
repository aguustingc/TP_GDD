...
CLIENTE_ID, 
FACTURA_FECHA, 
SUM((CAST(P.PRODUCTO_PRECIO AS DECIMAL(18,2)) * 1.2)) AS FACTURA_IMPORTE
FROM gd_esquema.Maestra M
JOIN SPARK.SUCURSAL S
ON M.SUCURSAL_DIR = S.SUCURSAL_DIR
...
CAST(ACCESORIO_CODIGO AS NVARCHAR(255)) AS DETALLE_CODIGO, COUNT(*) AS CANTIDAD, (CAST(P.PRODUCTO_PRECIO AS DECIMAL(18,2)) * 1.2) AS DETALLE_IMPORTE
FROM gd_esquema.Maestra M
JOIN SPARK.SUCURSAL S
ON M.SUCURSAL_DIR = S.SUCURSAL_DIR
JOIN SPARK.CLIENTE C
...
UNION ALL
SELECT FACTURA_NUMERO, SUCURSAL_ID, 
PC_CODIGO AS DETALLE_CODIGO, COUNT(*) AS CANTIDAD, (CAST(P.PRODUCTO_PRECIO AS DECIMAL(18,2)) * 1.2) AS DETALLE_IMPORTE
FROM gd_esquema.Maestra M
JOIN SPARK.SUCURSAL S
ON M.SUCURSAL_DIR = S.SUCURSAL_DIR
JOIN SPARK.CLIENTE C
