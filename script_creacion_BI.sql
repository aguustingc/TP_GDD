...
COMPRA_PLACA_VIDEO NVARCHAR(50) NOT NULL FOREIGN KEY REFERENCES SPARK.BI_DIM_PLACA_VIDEO(PLACA_ID),
...
VENTA_PLACA_VIDEO NVARCHAR(50) NOT NULL FOREIGN KEY REFERENCES SPARK.BI_DIM_PLACA_VIDEO(PLACA_ID),
...
SELECT prod.PRODUCTO_ID AS PRODUCTO, suc.SUCURSAL_ID AS SUCURSAL, clib.CLIENTE_ID AS CLIENTE, ti.TIEMPO_ID, dfac.DETALLE_NUMERO AS NUMERO_VENTA, dfac.DETALLE_CANTIDAD AS CANTIDAD, (prod.PRODUCTO_PRECIO*1.2) AS PRECIO, (dfac.DETALLE_CANTIDAD*prod.PRODUCTO_PRECIO*1.2) AS IMPORTE
FROM SPARK.DETALLE_FACTURA dfac INNER JOIN SPARK.FACTURA fac ON (dfac.DETALLE_NUMERO = fac.FACTURA_ID)
							    INNER JOIN SPARK.SUCURSAL suc ON (fac.FACTURA_SUCURSAL = suc.SUCURSAL_ID)
								INNER JOIN SPARK.CLIENTE cli ON (fac.FACTURA_CLIENTE = cli.CLIENTE_ID)
								INNER JOIN SPARK.PRODUCTO prod ON (dfac.DETALLE_CODIGO = prod.PRODUCTO_ID)
